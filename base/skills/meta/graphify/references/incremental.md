# Incremental Update and Cluster-only Modes

Companion reference for `skills/meta/graphify/SKILL.md`. Cover the `--update` and `--cluster-only` flows.
...
Before the merge step, save the old graph: `cp graphify-out/graph.json .graphify_old.json`. Clean up after: `rm -f .graphify_old.json`.
