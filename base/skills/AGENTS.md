...
Every new skill lands in either `base/skills/<category>/<name>/` (generic, vendor-neutral) or `overlays/team/skills/<category>/<name>/` (team-specific). Use the three-bucket policy:

1. **STRICT team** -> `overlays/team/skills/`. The skill is tied to internal services (error-tracking, CI, internal Kubernetes), uses team-only tools (VCS via team org, code-quality), or has workflows shaped by team's stack (AI Backend chat, MCP boundary). The `scope_boundary.py` check enforces this.
...
3. **HYBRID** -> split if practical (extract the team section into `overlays/team/skills/`); otherwise classify by PRIMARY AUDIENCE. Decision rule: "if a base-only user gets value from this skill, base; if value is only realized with team context, overlay."

`make new SKILL=<name> SCOPE=team` scaffolds under `overlays/team/skills/`; omit `SCOPE` (or `SCOPE=base`) for base.

## Do Not

- Add a skill that duplicates an existing one. Search `base/skills/`, `overlays/team/skills/`, `base/rules/`, and `overlays/team/rules/` first.
...
A skill in `base/skills/<cat>/<name>/SKILL.md` (or `overlays/team/skills/<cat>/<name>/SKILL.md`) only becomes "live" for an agent when all three layers agree.

| Layer | What it is for skills | Verified by |
|---|---|---|
| 1. Canonical source | The author's edit at `base/skills/<cat>/<name>/SKILL.md` (or `overlays/team/skills/<cat>/<name>/SKILL.md`), plus optional `references/` and `scripts/`. | `make check` (frontmatter + size + decay + audit) |
...
1. **Layer 1**: `cat base/skills/<cat>/<name>/SKILL.md` (or `overlays/team/skills/<cat>/<name>/SKILL.md`). Frontmatter present? `make check` clean?
